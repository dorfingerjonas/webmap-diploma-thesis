...
Eine der wichtigsten Vorteile einer Versionierung ist jedoch die Funktion, dass alte Stände von Dateien wiederhergestellen werden können.
...
besteht, dass eine wichtige Version überschreibt wird.
Die meisten Probleme werden von der automatischen Versionierung gelöst.
Bei dieser Variante werden automatisch, die als "fertig" markierte Stände, mit einer Versionsnummer versehen
und im Archiv schreibgeschützt abgelegt.
Dadurch hat werden die Risiken der manuellen Versionierung auf ein Minimum reduziert~\cite{versionierung}.
...
Dabei gibt es vier Stufen zwischen denen gewechselt werden kann.
...
Mit einem einfachen \emph{git clone} Befehl in der Konsole, kann das Repository geklont und somit ein lokales Repository erstellt werden.
...
In der Staging Area sind alle lokal modifizierten Files, welche für den nächsten Commit ausgewählt sind, gesammelt.
...
Eine semantische Versionsnummer besteht aus drei Teilen~\cite{semantische-versionierung}:
...
Durch viele Features von GitHub wird das Arbeiten mit git so stark erleichtert, dass auch Anfängerinnen und Anfänger
git in einer professionellen Umgebung kennenlernen und ausprobieren können.
Normalerweise wird git mit der Kommandozeile des jeweiligen Betriebssystemes verwendet, was hohes technisches Verständnis voraussetzt.
Mit einem kostenlosen Account können gratis Repositories angelegt und verwendet werden,
deshalb ist Github in der Open-Source Community sehr stark verbreitet.
Mittlerweile kann diese Software mehr als ''nur'' Projekte versionieren.
...
gratis Website Hosting und unter anderem die Möglichkeit die Dokumentation der geschriebenen Software besser zu bearbeiten~\cite{github-features}.

Zusätzlich gibt es auch andere Programme welche zu GitHub gehören.
2014 wurde \emph{Atom} vorgestellt, ein kostenloser Open-Source Editor für Entwicklerinnen und Entwickler, weiters gibt es noch \emph{Electron},
ebenfalls ein Open-Source JavaScript Framework.
Es wird verwendet um Web-Anwendungen zu Desktopanwendungen zu machen.
...
Dazu gibt es etliche Features, welche die Sicherheit, Rollen und zum Beispiel Berechtigungen betreffen.
...
Um Übersicht über den Projektverlauf zu behalten, ist die Entscheidung aufgrund interner Vereinbarungen von triply sowie
Erfahrungen des Teams auf GitHub Projects gefallen.
Ein Project Board kann ganz einfach über die GitHub Website erstellt werden, dabei gibt es die Auswahl ein vordefiniertes Template zu verwenden.
...
\begin{itemize}
    \item \textbf{To do:} einfache To dos, ohne großer Dringlichkeit
    \item \textbf{High priority:} Bugs welche möglichst schnell behoben werden müssen
    \item \textbf{Low priority:} Bugs welche in naher Zukunft behandelt werden müssen, jedoch nicht allzu dringend sind
    \item \textbf{Closed:} Wenn eine Aufgabe fertiggestellt ist, kommt diese in diese Spalte
\end{itemize}
...
Durch die Verwendung spezieller Keywords in \emph{commit messages} oder in einem Kommentar bei einem Issue, kann der Zustand dieses
Issues verändert werden.
Dies geht mit der Automatisierung von Project Boards einher.

\textbf{Keywords:} close, closes, closed, fix, fixes, fixed, resolve, resolves und resolved.
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[scale=0.35]{pics/austria-mapbox}
    \caption{Österreichische Staatsgrenzen mit Landesgrenzen dargestellt mit Mapbox}
    \label{fig:austria-mapbox}
\end{figure}

\subsection{Leaflet}
Leaflet ist laut eigenen Angaben führende Open-Source JavaScript Library, wenn mobile und interaktive Karten gefordert sind.
...
Dank einer leichten Anbindungsmöglichkeit für Plugins, gibt es etliche davon im Internet, die ganz einfach verwendet werden können,
um die Funktionalitäten zu erweitern.
Leaflet wird auch von den ganz großen Anbietern verwendet, zum Beispiel: GitHub, Pinterest, Facebook, European Commission und The Washington Post.
~\cite{leaflet-doc}

\begin{figure}[hbt!]
    \centering
    \includegraphics[scale=0.4]{pics/austria-leaflet}
    \caption{Österreichische Staatsgrenzen mit Landesgrenzen dargestellt mit Leaflet}
...
\begin{itemize}
    ...
    \item keine Unterstützung von Vektor Tiles, nur mit Plugins Dritter möglich
\end{itemize}

\subsection{Google Maps}
Google Maps ist der Kartendienst von Google und der Kartenprovider mit den meisten Daten, welche zur Verfügung stehen.
OpenStreetMap ist auf dem Globus nicht überall verfügbar, jeder kann aber an der Vergrößerung der Daten oder Informationen mitwirken.
Google Maps hingegen hat Daten bis in jede noch so kleine Straße, wodurch eine sehr gute Informationslage ermöglicht wird.
In Kombination mit PlacesAPI und der MapsImagesAPI können weder Leaflet noch Mapbox mit Google mithalten.
Das liegt auch daran, dass Google durch die hohen Marktanteile im gesamten digitalen Bereich, ein vielfaches an Daten zur Verfügung hat, als OpenStreetMap.
Der wohl größte Nachteil liegt an der kostenpflichtigen Nutzung ab einer gewissen Nutzerzahl.
Ab 1000 Anfragen pro Monat ist Google Maps nicht mehr gratis~\cite{google-maps-vs-osm}.

\begin{figure}[hbt!]
    \centering
    \includegraphics[scale=0.2]{pics/austria-googlemaps}
    \caption{Österreichische Staatsgrenzen mit Landesgrenzen dargestellt mit Google Maps}
...
\begin{itemize}
    ...
    \item Farbschema kann nur geringfügig verändert werden.
\end{itemize}
...
\begin{table}[hbt!]
    ...
    \caption{Map Frameworks Performance Vergleich}
\end{table}
...
Tile-layers bestimmen die Darstellung der Karte und werden von externen Servern direkt für den aktuell zu sehenden Bereich angefordert.
...
Das ist grundsätzlich möglich, kann aber eine Sicherheitslücke darstellen und es wird davon abgeraten~\cite{angular-interpolation}.
...
Bei Problemen und Fragen gibt es daher weniger Ressourcen, auf die zugegriffen werden kann.
...
Da Node.js plattformunabhängig ist, können diese in Skripts sowohl auf Windows als auch auf
...
Die meisten XML Dokumente enthalten außerdem eine sogenannte XML Declaration.
...
\begin{lstlisting}[label={lst:schema.xml}]{schema.xml}
...
Symbolic Links.
 ...
 Weiters kann damit ein bestimmter Code Style definiert werden, um den Code konsistent und lesbar zu machen.
...
\subsubsection{Image}
 ...
 und ein Befehl, um die Software im Container zu starten.
 ...
 \begin{lstlisting}[label={lst:Dockerfile}]{Dockerfile}
  FROM ubuntu:18.04
  COPY . /app
  RUN make /app
...
\subsection{Docker Compose}
 ...
 Datenbank zugreifen können, diese aber von Prozessen außerhalb nicht erreichbar ist.

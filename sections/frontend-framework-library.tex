...
Transpiling ist eine besondere Form des Compiling.
Ein Compiler nimmt Source Code in einer Programmiersprache und erzeugt daraus Source Code in einer anderen Sprache.
Transpiler machen grundsätzlich das gleiche, zeichnen sich aber dadurch aus, dass die Sprache, in die sie den Source Code übersetzen, ein
ähnliches Level an Abstraktion aufweist wie die Sprache, in der der originale Code geschrieben wurde.~\cite{compiling-vs-transpiling}

Ein Beispiel für einen Compiler ist der Java Compiler.
Dieser erzeugt aus Java Code sogenannten Java Bytecode, der anschließend von der Java Virtual Machine (JVM) ausgeführt werden kann.~\cite{java-bytecode}

Transpiler werden beispielsweise eingesetzt, um TypeScript Code in JavaScript Code umzuwandeln. Weitere Transpiler wandeln z.B. C++ in C,
CoffeeScript in JavaScript oder Dart in JavaScript um.~\cite{compiling-vs-transpiling}
